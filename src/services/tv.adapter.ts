...
export const tvAdapter = new Adapter<TvRequests>({
  ...
  if (id === null) { throw new Error('getShow: "id" is required'); }
  const res = await tvApi.get<Show>(`/shows/${id}`);
  return { data: res.data, status: res.status } as NetResult<Show>;
  },
  ...
  async getEpisodes(params) {
    ...
    if (typeof id !== 'number' || Number.isNaN(id)) {
      throw new Error('getEpisodes: "id" (show id) is required and must be a number');
    }
